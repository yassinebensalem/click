...
import { updateEditor, getEditors, getActiveEditorsByRole } from 'app/store/editorSlice';
import { unwrapResult } from '@reduxjs/toolkit';
import { getCountries } from 'app/store/countrySlice';
import { useSelector } from 'react-redux';
import { decode } from 'jsonwebtoken';

const EditEditor = ({ match }) => {
	...
	const validateRatioPriceError = ratioPrice => {
		...
		if (ratioPrice === '') {
			...
			setHelperTextRatioPrice('vous devez renseigner ce champ');
			setEnabled(false);
	...
	const validateRatioSellingError = ratioSelling => {
		if (ratioSelling === '') {
            ...
            setHelperTextRatioSelling('vous devez renseigner ce champ');
			setEnabled(false);
		}
